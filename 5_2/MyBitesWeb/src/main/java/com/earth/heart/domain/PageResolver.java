...
public class PageResolver {
	...
	private boolean showNext = false;	//다음페이지로 이동하는 링크를 보여줄지 여부 (endPage==totalPage이면 showNext는 false)
	...
	private void doPaging(int totalCnt, SearchItem sc) {
		this.totalPage = totalCnt/sc.getPageSize() + (totalCnt % sc.getPageSize() == 0 ? 0 : 1);       //전체 글 개수
		this.sc.setPage(Math.min(sc.getPage(), totalPage));			//page가 totalPage보다 크지 않음
		
		//현재 페이지(page)를 내비게이션 크기(NAV_SIZE)로 니누고, 다시 내비게이션 크기를 곱해서 1의 자리수를 0으로 만들고, 1를 더함
		this.beginPage = (this.sc.getPage()-1)/NAV_SIZE * NAV_SIZE + 1;
		
		//내비게이션 마지막 페이지 숫자
		//beginPage에 내비게이션 크기(NAV_SIZE)를 더하고 1 뺌 값과 총 페이지(totalPage)중에 적은 것을 구함
